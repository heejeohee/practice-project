...
private:
    int salary;
public:
    PermanentWorker(char * name, int money) : Employee(name)
    {salary=money;}
    int GetPay() const
    {
        return salary;
...
private:
    ...
    int payPerHour;
public:
    TemporaryWorker(char * name, int pay) : Employee(name)
    { workTime=0; payPerHour=pay;}
...
public:
    SalesWorker(char * name, int money, double ratio): PermanentWorker(name, money)
